...
# create logger
logger = logging.getLogger('2rhash_tda')
...
if __name__ == '__main__':
    ROUNDS = 2
    R = declare_ring([Block('x', 320 + 2*ROUNDS*320 + 64), 'u'], globals())
    ...
    diff = [[0] * 320 for i in range(2*ROUNDS-1)]
    ######### diff_pre ############
    #d0 = 0xe6765f2bfb737f78
    # pre diff0
    for i in range(64):
        diff[0][i] = R(x(2*ROUNDS*320 + 320 + i))
    # pre diff[1]
    d1 = 0xd255739452530b86
    for i in range(64):
        # diff[0][i] = d0 >> (63 - i) & 0x1
        diff[1][i] = d1 >> (63 - i) & 0x1
    logger.info(diff[1])
    ### Initialization ######
    Q = set()
    for i in range(320):
